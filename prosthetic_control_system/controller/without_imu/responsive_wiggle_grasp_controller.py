#!/usr/bin/env python3
"""
Responsive Wiggle Grasp Controller for Prosthetic Hand.

This controller extends the continuous_wiggle_grasp_controller by improving
responsiveness to proximity sensor readings. Key features:

1. Shorter phase durations (0.2-0.5 seconds) for faster cycling
2. Immediate phase exit when proximity thresholds are consistently triggered
3. Continuous random finger wiggling in ALL states
4. State-dependent wiggle amplitude scaling
5. Enhanced visualization with base position and wiggle component display
"""
...
logger = logging.getLogger("ResponsiveWiggleGrasp")
...
class ResponsiveWiggleGraspController:
    """
    Controller that implements continuous wiggling during ALL states with
    improved responsiveness to proximity triggers.
    ...
    When proximity thresholds are consistently triggered, the controller
    immediately exits the current phase to respond to the detection.
    """
    ...
    def __init__(
        ...
        min_phase_duration=0.2,  # Reduced from 0.5s
        max_phase_duration=0.5,  # Reduced from 2.0s
        max_angle=10.0,
        wiggle_speed=5.0,
        min_fingers=1,
        max_fingers=4,
        ...
        # Required consecutive readings
        readings_required=2        # Reduced from 3 for faster response
    ):
        """
        Initialize the responsive wiggle grasp controller.
        ...
        Args:
            ...
            readings_required: Consecutive readings needed for state change
        """
        # Save parameters
        ...
        self.readings_required = readings_required
        ...
        self.state_changed_this_cycle = False
        ...
        logger.info(f"Responsive wiggle grasp controller initialized with phase_duration={min_phase_duration}-{max_phase_duration}s")
    ...
    def _control_loop(self):
        """Main control loop that alternates between phases with responsive transitions"""
        ...
        while self.running and not self.shutdown_event.is_set():
            ...
            if current_time >= next_update_time:
                try:
                    # Reset state change flag at the beginning of each cycle
                    self.state_changed_this_cycle = False
                    
                    # Check if it's time to switch phases or if a force change was requested
                    if current_time >= self.next_phase_time or self.force_phase_change:
                        self.force_phase_change = False
                        self._toggle_phase()
    ...
    def _execute_sensor_phase(self):
        ...
        # This might force a phase change if detection criteria are met
    ...
    def _update_state(self, finger_distance, thumb_distance):
        """Update controller state based on sensor readings with immediate phase change capability"""
        current_time = time.time()
        ...
        state_transition_triggered = False
        
        if self.state == "IDLE":
            # Check if any finger is detecting an object
            if finger_distance < self.finger_threshold:
                self.finger_detection_count += 1
                logger.debug(f"Finger detection count: {self.finger_detection_count}/{self.readings_required} - distance: {finger_distance:.1f}mm")
            ...
            if thumb_distance < self.thumb_threshold:
                ...
                logger.debug(f"Thumb detection count: {self.thumb_detection_count}/{self.readings_required} - distance: {thumb_distance:.1f}mm")
        ...
        # If state transition was triggered and we're in SENSOR_PHASE,
        # force immediate switch to MOVEMENT_PHASE to react quickly
        if state_transition_triggered and self.current_phase == "SENSOR_PHASE":
            logger.info("🔥 IMMEDIATE PHASE CHANGE triggered by state transition")
            print("🔥 IMMEDIATE PHASE CHANGE triggered by state transition")
            self.force_phase_change = True
            self.state_changed_this_cycle = True
    ...
    def start(self):
        ...
        logger.info("Responsive wiggle grasp controller started")
    ...
    def stop(self):
        ...
        logger.info("Responsive wiggle grasp controller stopped")
        ...
        if current_time - self.last_status_update > 0.1:
            status = {
                ...
                "thresholds": {
                    ...
                    "readings_required": self.readings_required
                }
            }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Responsive Wiggle Grasp Controller')
    ...
    parser.add_argument('--min-phase', type=float, default=0.2,
                      help='Minimum phase duration in seconds (default: 0.2)')
    
    parser.add_argument('--max-phase', type=float, default=0.5,
                      help='Maximum phase duration in seconds (default: 0.5)')
    ...
    parser.add_argument('--readings', type=int, default=2,
                      help='Consecutive readings required for state change (default: 2)')
    ...
    try:
        ...
        controller = ResponsiveWiggleGraspController(
            ...
            readings_required=args.readings
        )
        ...
        # Monitor loop
        print("\nResponsive Wiggle Grasp Controller Active - Press Ctrl+C to stop")
        print("--------------------------------------------------------")
        print(f"This controller implements continuous wiggling with rapid proximity response:")
        ...
        print(f"\nFaster response with short phases ({args.min_phase}-{args.max_phase}s) and immediate phase switching")
        ...
        print(f"- Required consecutive readings: {args.readings}")
        ...
        while running:
            try:
                ...
                # Print status header
                print("\nResponsive Wiggle Grasp Controller")
                print("=================================")
