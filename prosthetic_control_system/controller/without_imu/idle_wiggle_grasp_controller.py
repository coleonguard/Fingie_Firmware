#!/usr/bin/env python3
"""
Idle Wiggle Grasp Controller for Prosthetic Hand.

This controller implements continuous random wiggling during the IDLE state,
and proximity-triggered grasping. Key features:

1. Fingers wiggle randomly when the hand is open (IDLE state)
2. When an object is detected, wiggling stops and thumb moves to opposition
3. When thumb detects the object, other fingers close to form a grasp
4. Clear separation between sensor reading and movement phases
5. Enhanced visualization of finger positions and sensor readings
"""
...
logger = logging.getLogger("IdleWiggleGrasp")
...
class IdleWiggleGraspController:
    """
    Controller that implements wiggling during IDLE state and
    ...
    2. THUMB_OPPOSING: Thumb in opposition, other fingers open
    ...
    def __init__(
        ...
        """
        Initialize the idle wiggle grasp controller.
        ...
        logger.info(f"Idle wiggle grasp controller initialized with phase_duration={min_phase_duration}-{max_phase_duration}s (uniform random)")
    ...
    def _execute_movement_phase(self):
        ...
        if self.state == "IDLE":
            self._execute_idle_wiggle()
        elif self.state == "THUMB_OPPOSING":
            self._position_thumb_for_opposition()
        ...
        if self.state == "IDLE":
            ...
            if finger_distance < self.finger_threshold:
                ...
                if self.finger_detection_count >= self.readings_required:
                    ...
                    self.finger_detection_count = 0
                    # Clear wiggling fingers when object is detected
                    self.wiggling_fingers = []
            else:
                self.finger_detection_count = 0
            ...
            if finger_distance > self.finger_threshold + self.hysteresis:
                ...
                if self.release_detection_count >= self.readings_required:
                    ...
                    self.state_entry_time = current_time
                    self.release_detection_count = 0
                    # Select new fingers to wiggle when returning to idle
                    self._select_random_fingers()
            else:
                self.release_detection_count = 0
    ...
    def _execute_idle_wiggle(self):
        ...
        # Make sure thumb and ThumbRotate are at 0
        try:
            ...
            logger.error(f"Error setting thumb position: {e}")
    
    def _position_thumb_for_opposition(self):
        """Position thumb in opposition during THUMB_OPPOSING state"""
    ...
    def start(self):
        ...
        logger.info("Idle wiggle grasp controller started")
    ...
    def stop(self):
        ...
        logger.info("Idle wiggle grasp controller stopped")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Idle Wiggle Grasp Controller')
    ...
    try:
        ...
        controller = IdleWiggleGraspController(
        ...
        # Monitor loop
        print("\nIdle Wiggle Grasp Controller Active - Press Ctrl+C to stop")
        print("----------------------------------------------------")
        print(f"This controller implements wiggling during IDLE state and proximity-triggered grasping:")
        print(f"1. IDLE: Fingers randomly wiggling, waiting for object detection")
        print(f"2. THUMB_OPPOSING: Thumb positioned at {args.thumb_oppose}° opposition, other fingers open")
        ...
        print(f"Will select {args.min_fingers}-{args.max_fingers} fingers to wiggle in IDLE state")
        print(f"Maximum wiggle angle: {args.max_angle}°, Maximum wiggle speed: {args.wiggle_speed}°/cycle")
        print(f"\nProximity Thresholds:")
        print(f"- Any finger < {args.finger_threshold}mm: Stop wiggling, position thumb")
        ...
        while running:
            try:
                ...
                for finger, data in status["fingers"].items():
                    ...
                    if finger != "Thumb" and status['state'] == "IDLE":
                        # During idle wiggling, show relative to max wiggle angle
