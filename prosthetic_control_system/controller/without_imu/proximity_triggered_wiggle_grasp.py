#!/usr/bin/env python3
"""
Proximity-Triggered Wiggle Grasp Controller for Prosthetic Hand.

This controller combines continuous random wiggling with proximity-triggered
grasping behavior. Key features:

1. Distinct states: IDLE, WIGGLE_APPROACH, GRASP_FORMING
2. Clear separation between sensor reading and movement phases
3. Randomized timing with uniform distribution
4. Thumb opposition during approach with finger wiggling
5. Full grasp formation when thumb detects object
6. Enhanced visualization of finger positions and sensor readings
"""
...
logger = logging.getLogger("WiggleGrasp")
...
class WiggleGraspController:
    ...
    1. IDLE: All fingers open, monitoring for objects
    2. WIGGLE_APPROACH: Thumb in opposition, other fingers wiggling
    3. GRASP_FORMING: All fingers closing to form a grip
    ...
    def __init__(
        ...
        """
        Initialize the wiggle grasp controller.
        ...
        logger.info(f"Wiggle grasp controller initialized with phase_duration={min_phase_duration}-{max_phase_duration}s (uniform random)")
    ...
    def _execute_movement_phase(self):
        ...
        if self.state == "IDLE":
            self._move_to_idle()
        elif self.state == "WIGGLE_APPROACH":
            self._execute_wiggle()
        ...
        if self.state == "IDLE":
            ...
            if finger_distance < self.finger_threshold:
                ...
                if self.finger_detection_count >= self.readings_required:
                    logger.info(f"Transitioning to WIGGLE_APPROACH (finger distance: {finger_distance:.1f}mm)")
                    print(f"\n🖐 OBJECT DETECTED - Transitioning to WIGGLE_APPROACH (distance: {finger_distance:.1f}mm)")
                    self.state = "WIGGLE_APPROACH"
                    self.state_entry_time = current_time
                    self.finger_detection_count = 0
                    
                    # Select random fingers to wiggle initially
            ...
            if thumb_distance < self.thumb_threshold:
                ...
                if self.thumb_detection_count >= self.readings_required:
                    ...
                    self.state_entry_time = current_time
                    self.thumb_detection_count = 0
                    self.wiggling_fingers = []  # Stop all wiggling
            else:
                self.thumb_detection_count = 0
            ...
            if finger_distance > self.finger_threshold + self.hysteresis:
                ...
                if self.release_detection_count >= self.readings_required:
                    logger.info(f"Returning to IDLE from WIGGLE_APPROACH (finger distance: {finger_distance:.1f}mm)")
    ...
    def _move_to_idle(self):
        """Move hand to idle position (all fingers open)"""
    ...
    def _execute_wiggle(self):
        """Execute wiggle behavior during WIGGLE_APPROACH state"""
        # Position thumb in opposition first
        try:
            ...
            thumb_flex = 5.0  # Minimal flexion
            self.motors.set_position("Thumb", thumb_flex)
            self.current_positions["Thumb"] = thumb_flex
            self.target_positions["Thumb"] = thumb_flex
        except Exception as e:
            logger.error(f"Error setting position for thumb: {e}")
        
        # Check if we need to select new random fingers every few wiggles
        if self.wiggle_count % 3 == 0 and random.random() < 0.5:
            self._select_random_fingers()
    ...
    def _execute_grasp(self):
        """Execute grasp formation during GRASP_FORMING state"""
    ...
    def start(self):
        ...
        logger.info("Wiggle grasp controller started")
    ...
    def stop(self):
        ...
        logger.info("Wiggle grasp controller stopped")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Proximity-Triggered Wiggle Grasp Controller')
    ...
    try:
        ...
        controller = WiggleGraspController(
        ...
        # Monitor loop
        print("\nProximity-Triggered Wiggle Grasp Controller Active - Press Ctrl+C to stop")
        print("-------------------------------------------------------------------")
        print(f"This controller combines wiggling with proximity-triggered grasping:")
        print(f"1. IDLE: All fingers open, waiting for object detection")
        print(f"2. WIGGLE_APPROACH: Thumb positioned at {args.thumb_oppose}° opposition, other fingers wiggling")
        print(f"3. GRASP_FORMING: All fingers closing to {args.finger_close}° to form a grip")
        ...
        print(f"- Any finger < {args.finger_threshold}mm: Trigger thumb opposition and wiggling")
        print(f"- Thumb < {args.thumb_threshold}mm: Trigger grasp formation")
        ...
        while running:
            try:
                ...
                # Print status header
                print("\nProximity-Triggered Wiggle Grasp Controller")
                print("========================================")
                ...
                # Print wiggle info if applicable
                if status['state'] == "WIGGLE_APPROACH":
                ...
                for finger, data in status["fingers"].items():
                    ...
                    if finger != "Thumb" and status['state'] == "WIGGLE_APPROACH":
                        # During wiggling, show relative to max wiggle angle
                        max_value = status["wiggle_info"]["max_angle"]
                    elif status['state'] == "GRASP_FORMING":
                        # During grasp, show relative to finger close angle
                    ...
                    if finger == "Thumb":
                        ...
                        thumb_rotate_target = status["fingers"].get("ThumbRotate", {}).get("target", 0.0)
                        
                        # Simplify the rotation display
                        rotation_percent = int(thumb_rotate / status["thresholds"]["thumb_opposition_angle"] * 10)
                        rotation_bar = "▓" * rotation_percent + "░" * (10 - rotation_percent)
